...
public class Animal {
    ...
    Animal(String name, boolean kind, String color, String gender) {
        this.name = name;
        this.kind = kind;
        this.color = color;
        this.gender = gender;
    ...
    public static void animalEating(Animal A1, Animal A2) {
        if (A1.kind != A2.kind) {
            ...
            if (A1.kind) {
                System.out.println("то зверь под названием " + A2.name + " съел бедного животного под названием " + A1.name);
            } else {
                System.out.println("то зверь под названием " + A2.name + " съел бедного животного под названием " + A1.name);
            }
        } else {
